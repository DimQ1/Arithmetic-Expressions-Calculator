module.exports = class Calculator {
    constructor() {
        this.operations = {
            ...
            '/': ({ firstValue, secondValue }) => {
                if (secondValue === 0) throw new Error('It is impossible to divide by zero');
    ...
    getCalculationPairOfValue(operationStack) {
    ...
    addNumberInOutputArray(element, outputArray) {
        ...
        if (numberElement < 0) {
            throw new Error(`you used ${element}, but you must use only a positive number!`);
        }
        outputArray.push(numberElement);
    }

    processLowPriorityOperator(operator, operationStack, outputArray) {
    ...
    processHigthPriorityOperator(operator, operationStack, outputArray) {
        if (operationStack.length === 0) {
            operationStack.push(operator);
        } else {
    ...
    calculateRPNExpression(expression) {
        this.checkTypeInputExpression(expression);
        ...
        inputArray.forEach((element) => {
            switch (element) {
                ...
                case '/':
                case '*':
                    operationStack.push(this.operations[element](this.getCalculationPairOfValue(operationStack)));
                    break;
                default:
    ...
    convertToRPN(expression) {
        ...
        inputArray.forEach((element) => {
            switch (element) {
                case '+':
                case '-':
                    this.processLowPriorityOperator(element, operationStack, outputArray);
                    break;
                case '/':
                case '*':
                    this.processHigthPriorityOperator(element, operationStack, outputArray);
                    break;
                case '(':
                    operationStack.push(element);
                    break;
                case ')':
                    ...
                    break;
                default:
                    this.addNumberInOutputArray(element, outputArray);
                    break;
            }
        });
